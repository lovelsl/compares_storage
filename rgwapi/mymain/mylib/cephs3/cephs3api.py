...
from mymain.mylib.config.configinfo.cephconfig import CephConfig
...
class CephS3Api():
    ...
    def _getCephInfo(self):
        cc = CephConfig()
        self.host = cc.getCephHost()
        self.port = cc.getCephPort()
        self.access_key = cc.getCephAccess()
        self.secret_key = cc.getCephSecret()
    ...
    def uploadBigFile(self, bucketname, filename, filepath):
        ...
        chunksize = CephS3Api.CHUNKSIZE #1024 * 1024 * 5 #5M
        ...
        for i in range(0, chunkcnt):
            ...
            with FileChunkIO(filepath, 'rb', offset=offset, bytes=len) as fp:
                print("len-->", len)
    ...
    def downloadBigFile(self, bucketname, filename, filepath):
        chunksize = CephS3Api.CHUNKSIZE   #1024 * 1024 * 5  # 2M
    ...
    def deletaAllDataInBuckets(self, bucketname):
        bucket = self.conn.get_bucket(bucketname)
        # print(dir(bucket))
        for key in bucket.list():
            #print(key)
            self.deleteFile(bucketname, key)
